...
작성 원칙:
- 도달목표, 수행과제, 평가기준을 종합하여 학생의 능력을 서술한다. 
- 문어체로 공식적인 단어를 선택하고 ~~임, ~함으로 끝냄. 
- 학교생활기록부 서술형 표현 양식 준수 (3인칭 서술, 과거형)
- 반드시 "과목" 단위로 통합해 작성한다 
— 같은 과목에 여러 채점기준이 있다면 모든 채점기준이 드러나도록 작성한다. 
...
const GRADE_LABEL: Record<"high" | "mid" | "low", string> = {
  high: "잘함",
  mid: "보통",
  low: "노력",
};
              ...
              .map((report) => {
                ...
                const itemLines = report.items
                  .map((item) => {
                    ...
                    const feedback = item.teacherFeedback
                      ? ` — "${item.teacherFeedback}"`
                      : "";
                    const criterion = item.criterionTitle
                      ? `${item.criterionTitle}: `
                      : "";
