...
export async function GET(_: Request, { params }: Params) {
  ...
  // 테이블별로 한 번씩만 읽는다.
  const [feedsRes, plansRes, checksRes, learningActRes, learningSubRes, surveyRes, lettersRes] = await Promise.all([
    supabaseAdmin
      .from('emotion_feeds')
  ...
  const latestSurveyId = surveyRes.data?.[0]?.id;

  if (latestSurveyId) {
    const { data: nominations } = await supabaseAdmin
      .from('relationship_nominations')
      .select('target_id,question_type')
      .eq('survey_id', latestSurveyId);

    const positiveTargets = new Set<string>();
    (nominations ?? []).forEach((nomination) => {
      if (nomination.question_type === 'role_isolated') isolated.add(nomination.target_id);
      if (nomination.question_type === 'positive') positiveTargets.add(nomination.target_id);
    });
