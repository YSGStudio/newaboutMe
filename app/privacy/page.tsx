/**
 * 개인정보처리방침 페이지 — 경로 "/privacy"
 * 별빛로그의 개인정보 수집·이용·보관·파기 방침을 조문 형태로 보여주는 정적 문서 페이지입니다.
 * 상단 상수(시행일/개정일/버전)와 부칙의 버전 이력을 함께 표시합니다. (원문: 별빛로그_개인정보처리방침.md)
 */
import Link from 'next/link';

const EFFECTIVE_DATE = '2026년 5월 14일';
const REVISION_DATE = '2026년 7월 28일';
    ...
    <div style={{ fontFamily: "'Pretendard', 'Noto Sans KR', sans-serif", background: '#fff', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: 'clamp(24px, 5vw, 56px) clamp(16px, 5vw, 32px)' }}>
        ...
        <Section title="제1조. 수집하는 개인정보 항목 및 수집 방법">
          ...
          <Table
            ...
            rows={[
              ...
              ['별빛메일', '편지 제목, 편지 내용', '학생이 직접 입력'],
              ['성찰일기', '자기 평가 내용', '학생이 직접 입력'],
              ['교우관계 설문', '문항별 지명 응답(선택한 친구), 개방형 응답 내용', '학생이 직접 입력'],
              ['뱃지·별빛 캐릭터', '뱃지 획득 기록, 별빛 캐릭터, 누적 뱃지 개수', '서비스 이용 실적에 따라 자동 부여'],
          ...
          <Note>
            ...
            ※ <strong>AI 분석 기능은 회원 등급별 월 사용 한도 내에서 제공됩니다</strong> (일반회원 월 10회, 유료회원 월 100회 · 매월 1일 초기화). AI 분석 실행 시 학생의 학습·감정·평가 데이터가 활용되며, 외부 AI 서비스(OpenAI)로 전송될 때 학생의 실명은 절대 포함되지 않습니다. 학생은 &apos;출석번호번 학생&apos;으로 익명화되어 전송됩니다.<br />
        ...
        <Section title="제2조. 개인정보의 수집 및 이용 목적">
          ...
          <Table
            ...
            rows={[
              ...
              ['학급 내 편지(별빛메일) 서비스', '편지 제목, 내용'],
              ['교우관계 분석 리포트 제공 (교사용)', '교우관계 설문 응답'],
              ['뱃지·별빛 캐릭터 부여 및 성장 동기 부여', '감정·계획·성찰·편지 이용 실적'],
        ...
        <Section title="제3조. 개인정보의 보유 및 이용 기간">
          <p style={{ fontSize: 14, color: '#374151', lineHeight: 1.8, marginBottom: 4 }}>
            수집된 개인정보는 서비스 이용 계약이 유지되는 동안 보유·이용하며, 회원 탈퇴(교사) 또는 학급·학생 삭제 시 아래 방법으로 파기합니다.
          </p>
          <Table
            headers={['항목', '보유 기간', '파기 방법']}
            rows={[
              ['교사 계정 정보', '회원 탈퇴 시까지', '복구 불가능한 방법으로 영구 삭제'],
              ['학생 정보 및 학습 기록', '교사가 학생 또는 학급을 삭제할 때까지', '즉시(5일 이내) 영구 삭제'],
              ['감정 피드, 계획, 별빛메일, 성찰일기, 교우관계 설문, 뱃지 기록, 평가 기록', '위와 동일', '위와 동일'],
        ...
        <Section title="제5조. 개인정보 처리 위탁 및 국외 이전">
          ...
          <InfoBox color="indigo">
            ...
            AI 분석 기능은 회원 등급별 월 사용 한도(일반회원 10회, 유료회원 100회) 내에서 담임교사가 실행합니다. OpenAI API를 통해 학생 데이터를 처리할 때 다음 보호 조치를 반드시 적용합니다.<br /><br />
        ...
        <Section title="제7조. 아동 개인정보 보호">
          ...
          <ul style={{ paddingLeft: 20, fontSize: 14, color: '#374151', lineHeight: 2 }}>
            ...
            <li>별빛메일은 같은 학급 학생 사이에서만 주고받을 수 있으며, 교사가 전체 내용을 확인·관리합니다. 이 사실은 서비스 이용 전 학생 및 보호자에게 안내됩니다.</li>
        ...
        <Section title="제8조. 개인정보의 안전성 확보 조치">
          ...
          <ul style={{ paddingLeft: 20, fontSize: 14, color: '#374151', lineHeight: 2 }}>
            ...
            <li>AI 분석 기능은 회원 등급별 월 사용 한도(일반회원 10회, 유료회원 100회)가 서버 수준에서 적용되며, 모든 실행 이력이 기록됩니다. 학급 전체 일괄 분석은 유료회원만 실행할 수 있습니다.</li>
        ...
        <Section title="제12조. 개인정보처리방침의 변경">
          ...
          <Table
            ...
            rows={[
              ...
              ['v1.3', '2026년 7월 12일', '운영 주체를 별빛에듀로 명시(총칙·제10조); 학생 비밀번호(숫자 4자리) 및 로그인 잠금 도입 반영(제1조·제7조·제8조); 교우관계 설문·뱃지 항목 추가(제1조·제2조·제7조); AI 사용 기록 수집 및 등급별 월 사용 한도(일반 20회/유료 100회) 반영(제1조·제2조·제5조·제8조)'],
              ['v1.4', '2026년 7월 28일', '일반회원 AI 분석 월 사용 한도를 20회에서 10회로 변경(제1조·제5조·제8조)'],
            ]}
          />
        </Section>
